    ...
    <div className="overflow-x-hidden">
      ...
      <section className="relative pt-32 pb-20 px-4 overflow-hidden">
        ...
        <Reveal className="relative max-w-4xl mx-auto text-center">
          <div className="eyebrow justify-center mb-6">Complete guide</div>
          <h1 className="headline-xl text-[var(--text-primary)] mb-6">
            How DineDash<br />
            <span className="text-[var(--brand)]">works</span>
          </h1>
          <p className="body-lead max-w-2xl mx-auto mb-8">
            Order first. Eat fast. Pay less. Here's everything you need to know about the DineDash dining experience.
          </p>
          {/* Mini tiers */}
          <div className="flex flex-wrap justify-center gap-3">
            {discountTiers.map((t) => (
              <div key={t.discount} className="bg-[var(--surface)] border border-[var(--border)] rounded-xl px-4 py-2 text-center shadow-sm">
                <span className={`font-bold text-lg ${t.color}`}>{t.discount}</span>
                <span className="text-[var(--text-muted)] text-sm ml-2">{t.time}</span>
              </div>
            ))}
          </div>
        </Reveal>
      ...
      <section className="px-4 -mt-8 mb-0 relative z-10">
        <Reveal delay={0.1} className="max-w-4xl mx-auto bg-[var(--surface)] border border-[var(--border)] rounded-3xl shadow-xl p-8 md:p-12">
          <div className="text-center mb-8">
            <div className="eyebrow justify-center mb-4">The core idea</div>
            <h2 className="headline-md text-[var(--text-primary)] mb-3">Order first. Eat fast. Pay less.</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
            {steps.map((step) => (
              <div key={step.num} className="flex flex-col items-center text-center">
                <div className="icon-tile mb-3 relative">
                  <step.icon className="w-6 h-6" />
                  <div className="absolute -top-1.5 -right-1.5 w-5 h-5 bg-[var(--surface)] border-2 border-[var(--border)] rounded-full flex items-center justify-center text-[10px] font-bold text-[var(--text-primary)]">
                    {step.num}
                  </div>
                </div>
      ...
      <section className="section-padding px-4 bg-[var(--surface-alt)]">
        <div className="max-w-4xl mx-auto">
          <Reveal className="text-center mb-14">
            <h2 className="headline-lg text-[var(--text-primary)] mb-3">Speed discount tiers</h2>
            <p className="text-[var(--text-muted)] text-lg">The faster you finish, the more you get back. These are the standard tiers — each restaurant can set their own.</p>
          </Reveal>
          <RevealStagger className="grid grid-cols-2 md:grid-cols-4 gap-4" stagger={0.1}>
            {discountTiers.map((t) => (
              <RevealItem key={t.discount} className={`card-premium border-2 p-6 text-center ${t.bg}`}>
                <div className={`text-4xl font-bold mb-1 ${t.color}`}>{t.discount}</div>
      ...
      <section className="section-padding px-4 bg-[var(--surface)]">
        <div className="max-w-4xl mx-auto">
          <Reveal className="text-center mb-16">
            <h2 className="headline-lg text-[var(--text-primary)] mb-3">Step-by-step breakdown</h2>
            <p className="text-[var(--text-muted)] text-lg">Every detail of the DineDash experience, explained.</p>
          </Reveal>
          <div className="space-y-8">
            {steps.map((step) => (
              <Reveal key={step.num} y={16} className="flex gap-6">
      ...
      <section className="section-padding px-4 bg-[var(--surface-alt)]">
        <div className="max-w-4xl mx-auto">
          <Reveal className="text-center mb-14">
            <h2 className="headline-lg text-[var(--text-primary)] mb-3">Everything explained</h2>
            <p className="text-[var(--text-muted)] text-lg">Tap any section to learn more about each part of DineDash.</p>
          </Reveal>
          <Reveal>
      ...
      <section className="section-padding px-4 bg-[var(--surface)]">
        <Reveal className="max-w-3xl mx-auto text-center">
          <h2 className="headline-lg text-[var(--text-primary)] mb-4">Ready to try it?</h2>
          <p className="text-[var(--text-secondary)] text-lg mb-8">Download DineDash and earn your first discount today. Quick sign-in, that's it.</p>
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <Link href="/get-started" className="btn-primary">
              Restaurant owner? Start here <ArrowRight className="w-5 h-5" />
            </Link>
            <Link href="/for-customers" className="btn-outline">
              I&apos;m a diner →
            </Link>
          </div>
        </Reveal>
